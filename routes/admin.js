...
// DASHBOARD
// ===========================================================================
router.get('/admin/whoami', auth, (req, res) => {
  res.json({ username: req.actor.username, role: req.actor.role });
});
...
// Hard delete - permanently removes the record, unlike cancel which just
// flags it. Admin-only since this can't be undone. Meant for cleaning up
// junk/test/duplicate rows and orphaned transactions (e.g. ones referencing
// a packageId that no longer exists in the catalog and will never fulfill
// correctly) rather than routine order handling - use Cancel for that.
router.delete('/admin/billing/:reference', auth, requireRole('admin'), async (req, res) => {
  try {
    const tx = await Transaction.findOneAndDelete({ reference: req.params.reference });
    if (!tx) return res.status(404).json({ message: 'Not found' });
    logActivity(req.actor.username, 'billing.delete', { reference: tx.reference, packageId: tx.packageId, phone: tx.phone });
    res.json({ ok: true });
  } catch (err) {
    console.error('DELETE /admin/billing failed:', err.message);
...
router.post('/router-stats', async (req, res) => {
  try {
    ...
    const { activeUsers, activeCount, totalVoucherAccounts, cpuLoad, freeMemory, totalMemory, uptime, totalBytesIn, totalBytesOut } = req.body;

    const existing = await RouterStats.findOne({ key: 'latest' });
    const history = existing ? existing.history || [] : [];
    history.push({ timestamp: new Date(), activeCount: activeCount || 0, cpuLoad: cpuLoad || 0, totalBytesIn: totalBytesIn || 0, totalBytesOut: totalBytesOut || 0 });
    ...
    await RouterStats.findOneAndUpdate(
      ...
      { activeUsers: activeUsers || [], activeCount: activeCount || 0, totalVoucherAccounts: totalVoucherAccounts || 0,
        cpuLoad: cpuLoad || 0, freeMemory: freeMemory || 0, totalMemory: totalMemory || 0, uptime: uptime || '',
        totalBytesIn: totalBytesIn || 0, totalBytesOut: totalBytesOut || 0, reportedAt: new Date(),
        history },
      { upsert: true }
    );
    res.json({ ok: true });
...
// Active Users - the raw router-stats snapshot only has what MikroTik's
// hotspot active table knows (user/uptime/address/mac/device/bytes). It has
// no idea what package that session belongs to, or which voucher/order it
// came from - that only exists on our side, keyed by hotspotUsername. This
// endpoint joins the two so the panel can show Package and link through to
// the full voucher record without the router needing to know anything more
// than it already does.
router.get('/admin/active-users', auth, async (req, res) => {
  try {
    const stats = await RouterStats.findOne({ key: 'latest' });
    const activeUsers = stats?.activeUsers || [];
    if (activeUsers.length === 0) {
      return res.json({ users: [], reportedAt: stats?.reportedAt || null });
    }

    const usernames = activeUsers.map(u => u.user).filter(Boolean);
    const txs = await Transaction.find({ hotspotUsername: { $in: usernames }, status: 'success' })
      .sort({ createdAt: -1 });
    // Most recent successful transaction per hotspotUsername - a username
    // could in theory be reused/re-billed, most recent is the relevant one.
    const txByUsername = {};
    for (const tx of txs) {
      if (!txByUsername[tx.hotspotUsername]) txByUsername[tx.hotspotUsername] = tx;
    }

    const users = activeUsers.map(u => {
      const tx = txByUsername[u.user];
      return {
        voucher: u.user,
        ...
        address: u.address,
        uptime: u.uptime,
        bytesIn: u.bytesIn ?? null,
        bytesOut: u.bytesOut ?? null,
    ...
    res.json({ users, reportedAt: stats?.reportedAt || null });
  } catch (err) {
    console.error('GET /admin/active-users failed:', err.message);
    res.status(500).json({ users: [] });
  }
});

// Disconnect - kicks a live session off the hotspot immediately. Reuses the
// existing RouterCommand queue (the same one Terminal and product-creation
// use) rather than needing any new router-side script - the router already
// polls for pending commands every 5s. Admin-only since this affects a
// paying customer's active connection.
router.post('/admin/active-users/disconnect', auth, requireRole('admin'), async (req, res) => {
  try {
    const { voucher, address } = req.body;
    if (!voucher) return res.status(400).json({ message: 'voucher (hotspot username) required' });
    const safeVoucher = String(voucher).replace(/"/g, '');
    const filter = address
      ? `[find user="${safeVoucher}" and address="${String(address).replace(/"/g, '')}"]`
      : `[find user="${safeVoucher}"]`;
    const command = `/ip hotspot active remove ${filter}`;
    const cmd = await RouterCommand.create({ command, requestedBy: req.actor.username });
    logActivity(req.actor.username, 'active-users.disconnect', { voucher: safeVoucher, address });
    res.json({ ok: true, id: cmd._id });
  } catch (err) {
    console.error('POST /admin/active-users/disconnect failed:', err.message);
    res.status(500).json({ message: 'Could not queue disconnect' });
  }
});
...
router.delete('/admin/products/:id', auth, requireRole('admin'), async (req, res) => {
  try {
    ...
    // Guard: pending/undispatched transactions still reference this exact
    // productId string to look up the correct voucher duration when the
    // router claims them. Deleting the product out from under them makes
    // claim-batch/claim-next silently fall back to a 5h default instead -
    // which is exactly the "no Product found for packageId" warnings in
    // the server logs. Block the delete instead of letting that happen
    // silently; deactivating (soft-delete) is always safe and doesn't
    // affect the lookup, since it's still findable by productId.
    const stuckCount = await Transaction.countDocuments({
      packageId: product.productId, status: 'success', synced: { $ne: true }
    });
    if (stuckCount > 0 && req.query.force !== 'true') {
      return res.status(409).json({
        message: `${stuckCount} pending voucher(s) still reference "${product.productId}" and haven't been fulfilled by the router yet. Deleting now would make them fall back to a 5h default duration. Wait for them to sync, delete/cancel those transactions in Billing first, or use Deactivate instead of Delete to keep it sellable-off but still resolvable.`,
        stuckCount
      });
    }
