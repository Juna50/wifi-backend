...
// API) and hands the client back an access_code. The frontend then uses
// InlineJS v2's resumeTransaction(access_code) to complete payment in a
// modal on the same page - Paystack's own recommended way to do this
// "without redirecting". This replaces the old flow where the browser
// called PaystackPop.setup() entirely client-side (v1, since deprecated)
// with nothing but a public key and a locally-made-up reference - which
// worked, but left Paystack's own checkout link idle as an unused fallback
// and meant the amount/email paystack saw was whatever the browser sent,
// not something we'd already told Paystack about server-side.
  ...
  try {
    ...
    try {
      const initRes = await fetch('https://api.paystack.co/transaction/initialize', {
        ...
        body: JSON.stringify({ email, amount: amountKobo, currency: 'GHS', reference })
      });
    ...
    res.json({ reference: tx.reference, amountKobo: tx.amountKobo, accessCode });
  } catch (err) {
